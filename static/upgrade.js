/**
 * Growth Experiment Lab — dApp Upgrade Wiring
...
(function() {
  ...
  async function apiFetch(path, opts = {}) {
    try {
      ...
      console.error('[Growth Experiment Lab] API error:', e);
  ...
  window.dappExportJSON = function(data, filename) {
    ...
    a.href = url;
    a.download = filename || '09-growth-lab-export.json';
    a.click();
  ...
  window.dappExportCSV = function(rows, filename) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = filename || '09-growth-lab-export.csv';
    a.click();
    URL.revokeObjectURL(url);
